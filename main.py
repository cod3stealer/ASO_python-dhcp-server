...
def root():
    if os.geteuid() != 0:
        ...
        print("Este script instalará el paquete ISC-DHCP-SERVER y manipulará varíos archivos de configuración\nDe esta manera, el script despliega un servicio DHCP sobre tu maquina")
        resp=input("Estás de acuerdo con esto? (si) / (no) ")
...
def prerequisitos_sockets():
    ...
    if c > 1:
        ...
        print(bcolors.HEADER + "\n\nLa interfaz se ha recogido correctamente:\n" + bcolors.HEADER + inet_internet + bcolors.ENDC + "\nCon IP" + bcolors.HEADER + os.popen(
                'ifconfig | grep ' + inet_internet + ' -A 1 | cut -d" " -f10').read() + bcolors.ENDC+bcolors.HEADER+"\nAHORA, selecciona una de las interfaces anteriores para que sea el encargado de dar conectividad a Internet\n"
                               "a los clientes escribiendo el nombre EXACTO de una de ellas." + bcolors.ENDC)

        interfaz = os.popen('nmcli dev | grep -v '+inet_internet+' | grep -w "conectado\|desconectado\|connected\|disconnected" | cut -d" " -f1,5').read()

        for i in interfaz.splitlines():
            print("\/ " + i + " \/" + os.popen('ifconfig | grep "' + i + '" -A 1 | cut -d" " -f10').read())
        print("AVISO: si escoges una interfaz de red desconectada, esta se activará automáticamente")

        interfaz = os.popen('nmcli dev | grep -v "' + inet_internet + '" | grep -w "conectado\|desconectado\|connected\|disconnected" | cut -d" " -f1').read()
        con = 0
        while con == 0:
            inet_dhcp = input("=> ")
            for i in interfaz.splitlines():
        ...
        result = os.popen('ip link show '+inet_dhcp+' up').read()
        if result == "":
            result = os.popen('ip link set '+inet_dhcp+' up').read()
            if result != 0:
                print(f"Error en: {inet_dhcp}.")
                exit(1)

        # Configuracion IP
        ip_dhcp = "192.168.71.1"
        os.popen('sudo ip addr flush dev '+inet_dhcp)
        result = os.popen('sudo ip addr add '+ip_dhcp+'/24 dev '+inet_dhcp).read()
        if result != "":
            print(f"Error: IP {ip_dhcp} en {inet_internet}.")
            exit(1)

        print(f"IP de {inet_dhcp} cambiada a {ip_dhcp}.")
        print(os.popen('ifconfig | grep "' + inet_internet + '" -A 1 | cut -d" " -f1,10').read())

        return inet_internet, inet_dhcp
    else:
...
def set_dhcp_conf(IP):
    ...
    with open("/etc/dhcp/dhcpd.conf", "w") as f:
        ...
        f.write("subnet {} netmask {}".format(subnet, netmask))
        f.write("{\n")
    ...
    os.popen('sudo service isc-dhcp-server start')
...
def set_static_dhcp(net,inet,inet_internet):
    net = net.strip()
    inet_internet = inet_internet.strip()
    netmanager = [
...
def __MAIN__():
    ...
    IP2 = os.popen('ifconfig | grep ' + inet[1] + ' -A 1 | cut -d" " -f10').read()
    print("Configurando archivos de servidor...")
    set_inetv4(inet[0])
    print("Asignando IP estática...")
    set_static_dhcp(IP,inet[0],IP2)
    print("Finalizando...")
    set_dhcp_conf(IP)
    print(bcolors.HEADER+"Servidor desplegado con éxito"+bcolors.ENDC)
